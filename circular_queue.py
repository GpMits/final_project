"""
Circular queue algorithm shell

Usage:
    circular_queue.py <number_of_robots_in_group> <group_colors> [--view]
...
def get_att_force_queue(robot, group_queue):
    angle = math.atan2(robot.pos[1]-TARGET_POS[1], robot.pos[0]-TARGET_POS[0])
    distance = get_distance(robot.pos, TARGET_POS)
    distance = distance - group_queue

    intensity = K_ATT*abs(distance)
    x_component = intensity*math.cos(angle)
    y_component = intensity*math.sin(angle)
    return x_component, y_component
        ...
        if group_states[i] == 1 or group_states[i] == 2:#Se o grupo está no estado entrando ou saindo do alvo
            ...
            for robot in robots:
                if robot.group == 'g' + str(i + 1):
                    ...
                    rep_force_x, rep_force_y = get_rep_force(robot)
                    att_force_x, att_force_y = get_att_force_queue(robot, group_points[i])

                    if get_distance(robot.pos, TARGET_POS) > TARGET_RADIUS + group_points[i]:
                        robot.pos[0] = robot.pos[0] + check_force(- att_force_x + rep_force_x)
                        robot.pos[1] = robot.pos[1] + check_force(- att_force_y + rep_force_y)
                    elif get_distance(robot.pos, TARGET_POS) < TARGET_RADIUS + group_points[i]:
                        robot.pos[0] = robot.pos[0] + check_force(att_force_x + rep_force_x)
                        robot.pos[1] = robot.pos[1] + check_force(att_force_y + rep_force_y)
                    else:
...
def main(args):
    ...
    for i in range(100):
        ...
        try:
            ...
            f = open('testes/circ/circ_{}_{}'.format(NUM_ROBOTS_IN_GROUP, len(GROUP_COLORS)), 'a')
